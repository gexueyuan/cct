 ...
 *
 * $Id: mac_misc.c 30017 2012-01-04 03:05:10Z yogesh.bellan $
 *
...
#ifdef NIBFromFlash
static void copy_mac_soft_reset(uint8_t init_pib);
#endif
/* === Implementation ====================================================== */
...
retval_t mac_init(void)
{
	...
	{
		copy_mac_soft_reset(true);
	}
	else copy_mac_soft_reset(false);
#else
    mac_soft_reset(true);
#endif
    gPowerUpFlag = false;
...
#ifdef NIBFromFlash
static void copy_init_pib(void)
{
	if (gPowerUpFlag == true)
	{
#if (MAC_ASSOCIATION_REQUEST_CONFIRM == 1)
    mac_pib.mac_AssociatedPANCoord = App_System_Para_Pointer->macInfo.mac_AssociatedPANCoord;
#endif /* (MAC_ASSOCIATION_REQUEST_CONFIRM == 1) */
#if ((MAC_INDIRECT_DATA_BASIC == 1) || defined(BEACON_SUPPORT))
    mac_pib.mac_MaxFrameTotalWaitTime = App_System_Para_Pointer->macInfo.mac_MaxFrameTotalWaitTime;
#endif  /* ((MAC_INDIRECT_DATA_BASIC == 1) || defined(BEACON_SUPPORT)) */
    mac_pib.mac_ResponseWaitTime = App_System_Para_Pointer->macInfo.mac_ResponseWaitTime;
    mac_pib.mac_SecurityEnabled = App_System_Para_Pointer->macInfo.mac_SecurityEnabled;

#if (MAC_ASSOCIATION_INDICATION_RESPONSE == 1)
    mac_pib.mac_AssociationPermit = App_System_Para_Pointer->macInfo.mac_AssociationPermit;
#endif /* (MAC_ASSOCIATION_INDICATION_RESPONSE == 1) */

#if (MAC_START_REQUEST_CONFIRM == 1)
    mac_pib.mac_BeaconPayloadLength = App_System_Para_Pointer->macInfo.mac_BeaconPayloadLength;
    mac_pib.mac_BSN = (uint8_t)rand();
#endif  /* (MAC_START_REQUEST_CONFIRM == 1) */

#if (MAC_INDIRECT_DATA_FFD == 1)
    mac_pib.mac_TransactionPersistenceTime = App_System_Para_Pointer->macInfo.mac_TransactionPersistenceTime;
#endif /* (MAC_INDIRECT_DATA_FFD == 1) */

    mac_pib.mac_AutoRequest = App_System_Para_Pointer->macInfo.mac_AutoRequest;
    mac_pib.mac_BattLifeExtPeriods = App_System_Para_Pointer->macInfo.mac_BattLifeExtPeriods;
    mac_pib.mac_CoordExtendedAddress = App_System_Para_Pointer->macInfo.mac_CoordExtendedAddress;
    mac_pib.mac_CoordShortAddress = App_System_Para_Pointer->macInfo.mac_CoordShortAddress;
    mac_pib.mac_DSN = (uint8_t)rand();
    mac_pib.mac_RxOnWhenIdle = App_System_Para_Pointer->macInfo.mac_RxOnWhenIdle;
                          ...
                          buffer_t *buf_ptr)
{
    ...
    csi->frame_msgtype = frame_msgtype;
