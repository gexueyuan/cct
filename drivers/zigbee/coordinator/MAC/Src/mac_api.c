 ...
 *
 * $Id: mac_api.c 30421 2012-01-24 10:05:01Z aroemer $
 *
...
/* === Prototypes ========================================================== */
extern void nwk_soft_reset(uint8_t init_pib);
extern void CheckSystemConfig(void);

/* === Implementation ====================================================== */
#ifdef SYS_OS
BSP_OS_SEM   zigbeeTask_Sem;
#endif
retval_t wpan_init(void)
{
    ...
    qmm_queue_init(&wait_nwk_ack_q);
#endif
    CheckSystemConfig();
    if (mac_init() == MAC_SUCCESS)
    {
      nwk_soft_reset(true);
#ifdef SYS_OS	  
      BSP_OS_SemCreate(&zigbeeTask_Sem,     1, "ZigbeeSE");
#endif	  
      return MAC_SUCCESS;
    }
    else return FAILURE;
...
#ifdef SYS_OS	
    else event_processed &=~0x08;
#endif    
    
    
    
    event = NULL;
    event = (uint8_t *)qmm_queue_remove(&nwk_aps_q, NULL);
...
//{
//	uartTxFlag = 0;
//	 pal_sio_tx(0, data, 65);//协调器发送自己的链路状态帧到pc
//}
#ifdef SYS_OS
    if (event_processed == 0)
    	BSP_OS_SemWait(&zigbeeTask_Sem, 0);
#endif		
