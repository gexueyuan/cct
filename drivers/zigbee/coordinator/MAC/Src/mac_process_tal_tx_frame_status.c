        ...
        case ASSOCIATIONRESPONSE:
            ...
            if ((tx_status == MAC_SUCCESS) || (tx_status == TAL_FRAME_PENDING))
            {
               ...
               // memcpy(&gMlmeCommStatus_DestAddr, frame->mpdu+PL_POS_DST_ADDR_START, sizeof(uint64_t));
                mac_mlme_comm_status(tx_status, frame->buffer_header);//没有判断noack等失败状态，是因为孩子还有可能会继续请求地址的，所以要保留间接队列里的信息
            }
            /* Set radio to sleep if allowed */
            mac_sleep_trans();
            break;
...
//#ifndef _ENDDEVICE_
//        	if (frame->NwkFrameHeader->frameControl.dstExtAddr == true)//让别人离开网络
//                nwk_gen_nlme_leave_conf(frame->buffer_header, tx_status, frame->NwkFrameHeader->field.dst_src.dstExt.value);
//            else
//#endif
//                nwk_gen_nlme_leave_conf(frame->buffer_header, tx_status, NULL);//让自己离开网络
//            mac_sleep_trans();
//
//             break;
             
//        case NWK_CMD_LEAVE_IND_FORWARD_N://父亲要移除自己的所有孩子，所以产生了离开转发帧
...
//#endif
					...
					if (frame->AppFrameHeader->frameControl.ackReq == true)
					{
						...
						timer_status =
							pal_timer_start(APP_TIMER_REQUEST_DATA,
											frame->AppFrameHeader->frameControl.timeOut*100000 + NWK_RETRY_TIMEOUT,//网络层ack最小延时500ms
											TIMEOUT_RELATIVE,
...
#ifndef _ENDDEVICE_             
        case NWK_DATA_IND_FORWARD_N://数据转发帧
#ifndef REDUCE_CHECK_PARMER1
        	if (gNwk_nib.deviceType != DEVICE_TYPE_END_DEVICE)
#endif
            {
            ...
            mac_sleep_trans();
             ...
             break;             
#endif             
        case NWK_DATA_RESPONSE_N://应用层数据回复帧
        ...
        case NWK_CMD_LINK_STATUS_N://链路状态帧
#ifndef REDUCE_CHECK_PARMER1
        	if (gNwk_nib.deviceType != DEVICE_TYPE_END_DEVICE)
#endif
            {
...
//                mac_sleep_trans();
        ...
        case NWK_CMD_ROUTE_REQUEST_RELAY_N:
            if (gRouteRelayRetries >= NWKC_RELAY_RREQ_RETRIES)
            {
                ...
                NwkState = NWK_MODULE_NONE;//要产生路由发现confirm
                bmm_buffer_free(frame->buffer_header);
                mac_sleep_trans();
