 ...
 *
 * $Id: mac_data_ind.c 30017 2012-01-04 03:05:10Z yogesh.bellan $
 *
...
void mac_process_tal_data_ind(uint8_t *msg)
{
    ...
    mac_parse_data.ppdu_RSSI         = frameptr->mpdu[mac_parse_data.mpdu_length + LQI_LEN + ED_VAL_LEN];
#ifdef PROMISCUOUS_MODE
...
static bool process_data_ind_not_transient(buffer_t *b_ptr, frame_info_t *f_ptr)
{
    ...
    switch (mac_state)
    {
            ...
            switch (mac_parse_data.frame_type)
            {
                case FCF_FRAMETYPE_MAC_CMD:
                {
                    switch (mac_parse_data.mac_command)
                    {
                        ...
                        case 0x55:
                        	if ( mac_parse_data.mac_payload_data.data.payload[0] == 0xAA &&
								 mac_parse_data.mac_payload_data.data.payload[1] == 0x03 &&
								 mac_parse_data.mac_payload_data.data.payload[2] == 0xC8 &&
								 mac_parse_data.mac_payload_data.data.payload[3] == 0x7F)
                        	{
        ...
        case MAC_PAN_COORD_STARTED://MAC_IDLE: cloudm
#if (MAC_ASSOCIATION_REQUEST_CONFIRM == 1)
        case MAC_ASSOCIATED:
#endif /* (MAC_ASSOCIATION_REQUEST_CONFIRM == 1) */
#if (MAC_START_REQUEST_CONFIRM == 1)
