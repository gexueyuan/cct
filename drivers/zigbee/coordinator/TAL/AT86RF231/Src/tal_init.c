...
#ifdef NIBFromFlash
static retval_t copy_internal_tal_reset(bool set_default_pib);
#endif
/* === IMPLEMENTATION ====================================================== */
...
retval_t tal_init(void)
{
	...
	{
	    if (copy_internal_tal_reset(true) != MAC_SUCCESS)
	    {
	        return FAILURE;
	    }
	}
	...
	{
	    if (copy_internal_tal_reset(false) != MAC_SUCCESS)
	    {
	        return FAILURE;
	    }
	}
          ...
          )
    {
        ...
        {
#ifdef STM32F10X_MD    
              *ptr_pib++ = *(uint8_t *)(0x1FFFF7EC+i);
#elif defined (STM32F051)
              *ptr_pib++ = *(uint8_t *)(0x1FFFF7AC+i);
#elif defined (STM32F4XX)
              *ptr_pib++ = *(uint8_t *)(0x1FFF7A10+i);              
#endif        
...
static retval_t trx_init(void)
{
    ...
    while ((pal_trx_reg_read(RG_VERSION_NUM) != AT86RF233_VERSION_NUM) ||
           (pal_trx_reg_read(RG_PART_NUM) != AT86RF233_PART_NUM));
...
static retval_t internal_tal_reset(bool set_default_pib)
{
...
#ifdef STM32F10X_MD    
    	seed += *(uint8_t *)(0x1FFFF7EC+i);
#elif defined (STM32F051)
    	seed += *(uint8_t *)(0x1FFFF7AC+i);
#elif defined (STM32F4XX)
    	seed += *(uint8_t *)(0x1FFF7A10+i);         
#endif           
        /*
         * Note:
         * Even if casting the 16 bit rand value back to 8 bit,
    ...
    srand(seed+TIM_GetCounter(TIM2));
#endif
#ifndef PA
    pal_trx_bit_write(SR_XTAL_TRIM, 15);
#endif
