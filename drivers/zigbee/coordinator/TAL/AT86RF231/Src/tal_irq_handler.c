...
void trx_irq_handler_cb(void)
{
    ...
    else if ((trx_irq_cause & TRX_IRQ_RX_START) &&      (App_System_Para_Pointer->AtCfgInfo.dobuleAntenna == true) )
    {
        /*
...
#endif
#else
    uint8_t trx_irq_cause,txStatus;
    trx_irq_cause = spi_sr(ISRSTS);
    if (trx_irq_cause & 0x01)
    {
        /*
         * TRX_END reason depends on if the trx is currently used for
         * transmission or reception.
...
#endif
        {
        	txStatus = spi_sr(TXSR);
            /* Get the result and push it to the queue. */
          //  if (trx_irq_cause & 0x01)
            {
                handle_tx_end_irq(txStatus);            // see tal_tx.c
            }
          //  else
          //  {
          //      handle_tx_end_irq(false);            // see tal_tx.c
...
//        }
    ...
    if (trx_irq_cause & 0x08)
    	if (tal_state != TAL_TX_AUTO)
        {
          	/* Turn off reception of packets off the air. This prevents the
	 * device from overwriting the buffer while we're reading it. */
//	txStatus = spi_sr(BBREG1);
//	txStatus |= 4; /* SET RXDECINV */
//	spi_sw(BBREG1, txStatus);
    		handle_received_frame_irq();    // see tal_rx.c
//	txStatus = spi_sr(BBREG1);
//	txStatus &= ~0x4; /* Clear RXDECINV */
//	spi_sw(BBREG1, txStatus);            
        }
#endif
}/* trx_irq_handler_cb() */
