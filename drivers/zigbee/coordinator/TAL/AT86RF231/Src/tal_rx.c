 ...
 *
 * $Id: tal_rx.c 31132 2012-03-02 15:04:10Z yogesh.bellan $
 *
...
void handle_received_frame_irq(void)
{
...
#else
    //ed_value = spi_lr(RSSIVALUE);
    phy_frame_len = spi_lr(RXBASE);
#endif
     ...
     */
    ext_frame_length = 127 + LENGTH_FIELD_LEN + LQI_LEN + ED_VAL_LEN;//TODO 改为固定127个字节 phy_frame_len
    ...
    receive_frame->mpdu[phy_frame_len + LQI_LEN + ED_VAL_LEN] = 84-ed_value;//ed指示0到84范围，84是离得最近的时候
#else
    spi_rd_rx_fifo(frame_ptr, LENGTH_FIELD_LEN + phy_frame_len + LQI_LEN + ED_VAL_LEN);
    receive_frame->mpdu = frame_ptr;
    receive_frame->mpdu[phy_frame_len + LQI_LEN + ED_VAL_LEN] = 255 - receive_frame->mpdu[phy_frame_len + LQI_LEN + ED_VAL_LEN];
    /* Add ED value at the end of the frame buffer. */
    //receive_frame->mpdu[phy_frame_len + LQI_LEN + ED_VAL_LEN] = 84-ed_value;//ed指示0到84范围，84是离得最近的时候
#endif
