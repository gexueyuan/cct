...
#define MUTABLE_PART(neighbor) (&(neighbor->mutableTable))
 ...
 ******************************************************************************/
INLINE NwkMutablePartOfNeighbor_t* nwkMutablePart(NwkNeighbor_t const *const neighbor)
{
  uint32_t a,b,c;
  a = (uint32_t)&(neighborTableArray[0].table.mutableTable);
  b = (uint32_t)&(neighborTableArray[0].table);
  c = a-b;
  c = (uint32_t)neighbor+c;
  return (NwkMutablePartOfNeighbor_t *)c;
}
