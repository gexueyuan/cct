...
extern volatile int  g_next, g_pervious;
extern unsigned char volmn;
extern void decoding_state_set(int state);
...
// 普通按键处理
	...
	{	
		...
		{
			if (decoding_state_get())
				decoding_state_set(0);
			else
			{
				decoding_state_set(1);
			}	
		}	
		//	key->kbd_event.key  = RTGUIK_UP;
